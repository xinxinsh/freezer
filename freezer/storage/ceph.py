...
class CephStorage(physical.PhysicalStorage):
    ...
    def _backup_metadata(self, headers):
    ...
    def _get_backup_base_name(backup_name):
        return freezer_utils.convert_str("%s.backup.base" % backup_name)

    def _snap_exist(self, rbd_image, snap_name):
        try:
            snaps = rbd_image.list_snaps()
        finally:
            rbd_image.close()
    ...
    def _delete_base_image(self, backup_name, snap_name, client):
        ...
        while retries > 0:
            snap_exist = self._snap_exist(base_image, snap_name)
            if snap_exist:
    ...
    def backup(self, connection_info, backup_name, headers=None):
        ...
        if base_name not in rbd.list(ioctx=dst_client.ioctx):
            if from_snap:
                LOG.debug("source snapshot {0} of volume {1} is stale or deleting", from_snap, src_name)
                src_image.remove_snap(from_snap)
                from_snap = None
            self._create_base_image(base_name, size, dst_client)
            image_created = True
        else:
            if not self._snap_exist(src_image, from_snap):
        ...
        try:
            self._rbd_diff_transfer(src_name, src_pool, base_name, 
                                    ...
                                    dst_user=self.ceph_backup_user,
                                    dst_conf=self.ceph_backup_conf,
            ...
            LOG.debug("Deleting diff backup snapshot %s from source volume",
                      %(new_snap, src_name))
    ...
    def get_backups(self, pool, prefix):
        ...
        with RADOSClient(self, pool) as client:
            ...
            if base_name in backups:
                ...
                rbd_image = rbd.Image(client.ioctx, base_name)
                snaps = rbd_image.list_snaps()
                rbd_image.close()
                backups = list(filter(lambda x: x.rsplit("_", 2)[0] == prefix, backups))
            else:
                # full backup
    ...
    def create_image(self, backup_name):
        ...
        with RADOSClient(self, self.ceph_backup_pool) as client:
		    ...
		    snap_exist = self._snap_exist(src_rbd, snap_name)
		    if snap_exist:
			errmsg = (_("Snapshot='%(snap)s' does not exist " % {'snap': snap_name})
			LOG.info(errmsg)
			raise exceptions.SnapshotNotFound(errmsg)
                ...
                src_rbd = self.rbd.Image(client.ioctx, base_name, 
                                         src_snap=snap_name, read_only=True)
            else:
